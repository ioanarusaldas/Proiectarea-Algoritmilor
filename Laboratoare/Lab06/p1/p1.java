...
class Nim {
    ...
    public boolean ended() {
         ...
         */
        int ok = 0;//toate mutarile s-au terminat
        for (int i = 0; i < 3; i++  ) {
        	if(heaps[i] != 0)
        	ok =1;
        }
        if(ok == 0)//jocul s-a terminat
        	return true;
        return false;
    }
    ...
    public boolean apply_move(Move move) {
        ...
        if(heaps[move.heap] < move.amount)
            return false;
        heaps[move.heap] = heaps[move.heap] - move.amount;
        return true;
    }
...
class P1 {
    ...
    public static Pair<Integer, Move> minimax(Nim init, int player, int depth) {
         ...
         */
        if ((depth <= 0 ) || (init.ended() == true))
            return new Pair<Integer, Move>(init.eval(player), new Move(0, 0));
         ...
         */
        int max_score = (-1) * Nim.Inf;
        int index = 0;
        int score;

        for(int i = 0; i < moves.size(); i++) {

            Nim move_cloned = (Nim)init.clone();

            score = (-1)*(minimax(move_cloned, player, depth - 1).first);
        ...
        Move best_move = moves.get(index);

        return new Pair<Integer, Move>(max_score, new Move(best_move.amount, best_move.heap));
    }
